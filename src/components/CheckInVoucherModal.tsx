...
export const CheckInVoucherModal: React.FC<CheckInVoucherModalProps> = ({ reservation, onClose }) => {
  ...
  // Add reservation card to Google Wallet & Calendar
  const handleAddToGoogleWallet = () => {
    try {
      ...
      const startStr = formatGoogleDate(inDate);
      const endStr = formatGoogleDate(outDate);

      const title = encodeURIComponent(`🌴 Estancia en Villa María (Reserva #${reservation.id.slice(0, 8)})`);
      const details = encodeURIComponent(
        `🎟️ PASE DIGITAL DE ACCESO & CHECK-IN - VILLA MARÍA\n\n` +
        `👤 Huésped: ${reservation.guestName}\n` +
        `🔑 Código de Reserva: ${reservation.id}\n` +
        `📶 Wi-Fi Red: ${wifiSsid}\n` +
        `🔑 Wi-Fi Contraseña: ${wifiPass}\n` +
        `📍 Dirección: ${address}\n` +
        `🛡️ Garita: Presentar este código en la garita de seguridad de la Calle 15\n` +
        `📞 Anfitrión / WhatsApp: ${hostPhone}\n` +
        `🗺️ Google Maps: ${mapsUrl}`
      );
      const location = encodeURIComponent(address);

      // 1. Google Wallet / Google Calendar direct sync URL
      ...
      // 2. Generate and download .ics Pass (for Google Wallet / Apple Wallet / Android)
      const icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Villa Maria//Digital Boarding Pass//ES',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        'BEGIN:VEVENT',
        `UID:villamaria-${reservation.id}@villamaria.com`,
        `DTSTAMP:${formatGoogleDate(new Date())}`,
        `DTSTART:${startStr}`,
        `DTEND:${endStr}`,
        `SUMMARY:🌴 Estancia en Villa María (Reserva #${reservation.id.slice(0, 8)})`,
        `DESCRIPTION:Pase de Entrada - Villa María\\nCódigo: ${reservation.id}\\nHuésped: ${reservation.guestName}\\nWi-Fi: ${wifiSsid} / ${wifiPass}\\nGarita: Calle 15\\nContacto: ${hostPhone}`,
      ...
      link.setAttribute('download', `Pase-VillaMaria-${reservation.id.slice(0, 8)}.ics`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      // Open Google Calendar / Wallet Web link
      window.open(gcalUrl, '_blank', 'noopener,noreferrer');

      setAddedWallet(true);
      setTimeout(() => setAddedWallet(false), 3500);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-emerald-950/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 overflow-y-auto print:p-0 print:bg-white">
      <div className="bg-emerald-950 sm:border sm:border-emerald-500/30 sm:rounded-3xl max-w-xl w-full p-3 sm:p-6 shadow-2xl relative my-2 text-emerald-100 font-sans max-h-[96vh] flex flex-col print:border-none print:shadow-none print:max-h-none print:p-0 print:bg-white">
        ...
        {/* Google Wallet Added Success Toast */}
        {addedWallet && (
          <div className="mt-2.5 p-2.5 rounded-xl bg-emerald-500/20 border border-emerald-400/40 text-emerald-200 text-xs flex items-center gap-2 print:hidden animate-fade-in">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
            <span>✅ Pase preparado para Google Wallet. Abriendo sincronización y descarga de tarjeta (.ics).</span>
          </div>
        )}

        {/* Action Buttons (Google Wallet, Download PNG, Print PDF, Close) - Hidden in Print */}
        <div className="pt-3 border-t border-emerald-500/20 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 shrink-0 print:hidden">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-black hover:bg-neutral-900 text-white font-bold text-xs uppercase tracking-wider transition-all shadow-md active:scale-95 border border-neutral-700 min-h-[44px]"
              title="Añadir a Google Wallet y Calendario"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-bold text-xs uppercase tracking-wider transition-all shadow-md active:scale-95 disabled:opacity-50 min-h-[44px]"
            >
              <Download className="w-4 h-4" />
              <span>{downloading ? 'Generando...' : 'Guardar PNG'}</span>
            </button>

            {/* Print / PDF Button */}
            <button
              onClick={handlePrint}
              className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-emerald-900/60 hover:bg-emerald-800 text-emerald-200 border border-emerald-500/30 font-bold text-xs uppercase tracking-wider transition-all active:scale-95 min-h-[44px]"
            >
